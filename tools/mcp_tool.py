        ...
        if which_hit:
            ...
            try:
                ...
                hermes_home = os.path.expanduser(
                    os.getenv(
                        "PROTEINCLAW_HOME",
                        os.getenv("HERMES_HOME", os.path.join(os.path.expanduser("~"), ".proteinclaw")),
                    )
                )
            candidates = [
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the ProteinClaw config file.
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves ProteinClaw's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the ProteinClaw registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Does NOT register tools in the ProteinClaw registry.
