...
ProteinClaw keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for ProteinClaw's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with ProteinClaw)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). ProteinClaw typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with ProteinClaw retry/error classification."""
