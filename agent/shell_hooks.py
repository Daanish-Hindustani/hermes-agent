...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # ProteinClaw-canonical
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a ProteinClaw wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical ProteinClaw
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ ProteinClaw is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    holding the canonical ProteinClaw-wire-shape response."""
