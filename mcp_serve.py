"""
ProteinClaw MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (ProteinClaw-specific extra)
...
class EventBridge:
    ...
    This is the ProteinClaw equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the ProteinClaw MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "ProteinClaw messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ProteinClaw MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
