"""ACP agent server — exposes ProteinClaw via the Agent Client Protocol."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping ProteinClaw AIAgent."""
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show ProteinClaw version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show ProteinClaw version",
        },
    )
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate ProteinClaw using the currently configured {provider} runtime credentials.",
                )
            ]
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # ProteinClaw' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run ProteinClaw on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"ProteinClaw v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when ProteinClaw has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
