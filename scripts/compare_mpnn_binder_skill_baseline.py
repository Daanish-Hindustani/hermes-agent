#!/usr/bin/env python3
"""Compare baseline and candidate ProteinMPNN binder skill performance."""
...
from experiments.mpnn_binder_skill_optimization.hermes_runner import load_text, run_candidate
from experiments.mpnn_binder_skill_optimization.scoring import score_dataset
...
def main() -> int:
    ...
    baseline = _evaluate(load_text(args.baseline), reference, scenarios, args.model, args.provider, args.api_mode)
    candidate = _evaluate(load_text(args.candidate), reference, scenarios, args.model, args.provider, args.api_mode)
    comparison = {
        "baseline": baseline,
        "candidate": candidate,
    ...
    write_markdown_report(out_dir / "baseline_report.md", "ProteinMPNN Binder Skill Baseline", baseline)
    write_json_report(out_dir / "comparison_report.json", comparison)
    write_markdown_report(out_dir / "comparison_report.md", "ProteinMPNN Binder Skill Comparison", comparison)
