...
app = FastAPI(title="ProteinClaw", version=__version__)
...
_SESSION_HEADER_NAME = "X-ProteinClaw-Session-Token"
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ProteinClaw supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    ProteinClaw resolves Anthropic creds in this order at runtime:
    1. ``~/.hermes/.anthropic_oauth.json`` — ProteinClaw-managed PKCE flow
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"ProteinClaw PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into ProteinClaw even
    when they also have a separate ProteinClaw-managed PKCE login.
    """
    try:
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same ProteinClaw-managed PKCE file
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both ProteinClaw file AND credential pool.
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "ProteinClaw Teal",  "description": "Classic dark teal — the canonical ProteinClaw look"},
...
def start_server(
    ...
    *,
    ...
    print(f"  ProteinClaw Web UI → http://{host}:{port}")
