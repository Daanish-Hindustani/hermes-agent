...
def run_backup(args) -> None:
    """Create a zip backup of the ProteinClaw home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: ProteinClaw home directory not found at {hermes_root}")
        sys.exit(1)
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a ProteinClaw backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a ProteinClaw backup "
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has ProteinClaw configuration.")
        ...
        print("Done. Your ProteinClaw configuration has been restored.")
