#!/usr/bin/env python3
"""
ProteinClaw CLI - Main entry point.
...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → ProteinClaw + Honcho
    hermes version             Show version
    hermes update              Update to latest version
    hermes uninstall           Uninstall ProteinClaw
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether ProteinClaw itself has been explicitly configured (model
    ...
    # Only count these if ProteinClaw has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants ProteinClaw to use their tokens.
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like ProteinClaw isn't configured yet -- no API keys or providers found."
        )
        print()
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with ProteinClaw?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ProteinClaw'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
#
# ProteinClaw uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  ProteinClaw only falls back to a lightweight backend (OpenRouter,")
...
def _model_flow_custom(config):
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"ProteinClaw will still save it."
        )
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  ProteinClaw will probe your")
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates ProteinClaw turns to `copilot --acp`.")
    print("  ProteinClaw currently starts its own ACP subprocess for each request.")
    print("  ProteinClaw uses your selected model as a hint for the Copilot ACP session.")
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   ProteinClaw typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with ProteinClaw, enable billing on your "
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    ProteinClaw will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate ProteinClaw CLI with a provider."""
...
def cmd_backup(args):
    """Back up ProteinClaw home directory to a zip file."""
...
def cmd_import(args):
    """Restore a ProteinClaw backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"ProteinClaw v{__version__} ({__release_date__})")
...
def _update_via_zip(args):
    """Update ProteinClaw by downloading a ZIP archive.
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if ProteinClaw behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but ProteinClaw couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but ProteinClaw couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if ProteinClaw behaves unexpectedly.")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official ProteinClaw repository.")
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# ProteinClaw — ensure /usr/local/bin is on PATH "
...
def cmd_update(args):
    """Update ProteinClaw to the latest version.
    ...
    if is_managed():
        managed_error("update ProteinClaw")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Warn if legacy ProteinClaw gateway unit files are still installed.
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy ProteinClaw gateway unit(s) detected:")
...
def cmd_logs(args):
    """View and filter ProteinClaw log files."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="hermes",
        description="ProteinClaw - AI assistant with tool-calling capabilities",
...
""",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with ProteinClaw",
    )
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        ...
        description=(
            "Stop, disable, and remove legacy ProteinClaw gateway unit files "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure ProteinClaw with an interactive wizard. "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for ProteinClaw.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "ProteinClaw")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for ProteinClaw CLI",
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate ProteinClaw with Spotify via PKCE")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of ProteinClaw components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with ProteinClaw setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your ProteinClaw setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for ProteinClaw. Use 'hermes debug share' to "
...
""",
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up ProteinClaw home directory to a zip file",
        description="Create a zip archive of your entire ProteinClaw configuration, "
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a ProteinClaw backup from a zip file",
        description="Extract a previously created ProteinClaw backup into your "
        "ProteinClaw home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage ProteinClaw configuration",
    )
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run ProteinClaw as an MCP server",
        description=(
            "Manage MCP server connections and run ProteinClaw as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'hermes mcp add' to connect to a new server, or\n"
            "'hermes mcp serve' to expose ProteinClaw conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run ProteinClaw as an MCP server (expose conversations to other agents)",
    )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to ProteinClaw",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to ProteinClaw",
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update ProteinClaw to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall ProteinClaw",
        description="Remove ProteinClaw from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run ProteinClaw as an ACP (Agent Client Protocol) server",
        description="Start ProteinClaw in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch ProteinClaw as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated ProteinClaw instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the ProteinClaw web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter ProteinClaw log files",
