# Contributing to ProteinClaw

Thank you for contributing to ProteinClaw! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want ProteinClaw to work everywhere.
...
Bundled skills (in `skills/`) ship with every ProteinClaw install. They should be **broadly useful to most users**:
...
The user may skip setup and keep loading the skill. ProteinClaw only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing ProteinClaw tools (`web_extract`, `terminal`, `read_file`).
...
ProteinClaw uses a data-driven skin system — no code changes needed to add a new skin.
...
ProteinClaw runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
ProteinClaw has terminal access. Security matters.
...
- Include: OS, Python version, ProteinClaw version (`hermes version`), full error traceback
